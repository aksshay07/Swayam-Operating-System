...
org 0x7c00        ; organize from 0x7C00 memory location where BIOS will load os
...
start:              ; start label from where our code starts
    ...
    start_os_intro db 'Welcome to Swayam OS!',0
    press_key db '>>>> Press any key <<<<',0

    login_username db 'Username : ',0
    login_password db 'Password : ',0
    
    display_text db 'VIT Bhopal', 0

    os_info db 10, 'Swayam Operating System, 16-Bit, version=1.0.0',13,0

    press_key_2 db 10,'Press any key to go to graphics view',0

    window_text db 10,'Graphics in Swayam OS', 0
    hello_world_text db 10,10, '    Hello Team Swayam!',0
